...
int main(int argc, char **argv)
{
    ...
    pcl::VoxelGrid<pcl::PointXYZ> vg;            //Downsampling a Pointcloud using a VoxelGrid Filter
    vg.setLeafSize (0.01f, 0.01f, 0.01f);        //set voxel unit      //leaf size 1cm

    pcl::search::KdTree<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ>);     //kDTree allows spatially close points to be stored in memory close together, which simplifies the algorithm and reduces computation time that would be spent repeatedly iterating through an unordered list.
    ...
    while(ros::ok())
    {
        ...
        // Apply voxel filter
        vg.setInputCloud (cloud.makeShared());        //input pointcloud
        vg.filter (*cloud_filtered);                  //output pointcloud 
