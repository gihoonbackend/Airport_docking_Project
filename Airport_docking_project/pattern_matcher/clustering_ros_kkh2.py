...
class Clustering():
    def __init__(self):
        ...
        self.sub=rospy.Subscriber('/laserPointCloud',PointCloud2,self.pcl_callback,queue_size=1)
    ...
    def get_colored_clusters(self,clusters, cloud):
    ...
    def pcl_callback(self,pcl_msg):
        ...
        cloud = self.do_voxel_grid_downsampling(cloud,0.01) 
        ...
        clusters = self.get_clusters(colorless_cloud, tolerance = 0.05, min_size = 100, max_size = 1500)

        # Assign a unique color float for each point (x, y, z)
        # Points with the same color belong to the same cluster
        colored_points = self.get_colored_clusters(clusters, colorless_cloud)
        ...
        clusters_out=self.laserProj.ProjectLaser(clusters_msg)
        
        clusters_pub.publish(clusters_out)
